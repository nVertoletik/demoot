...
ENVIRONMENT_ID=$(echo "$url" | sed -E 's#https://([^.]+).*#\1#')

echo "docker login https://$ONEAGENT_ENVIRONMENT_HOST --username $ENVIRONMENT_ID --password $ONEAGENT_INSTALLER_DOWNLOAD_TOKEN"
